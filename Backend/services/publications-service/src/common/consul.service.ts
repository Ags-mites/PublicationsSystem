...
export class ConsulService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor() {
    this.serviceId = (process.env.SERVICE_NAME || 'publications-service') + '-' + Math.random().toString(36).substring(7);
  ...
  async onModuleInit() {
    try {
      const serviceName = process.env.SERVICE_NAME || 'publications-service';
  ...
  // Método para descubrir otros servicios
